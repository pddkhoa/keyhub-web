...
const Header = () => {
    ...
    const handleLogout = () => {
        sendRequest({
            type: REQUEST_TYPE.LOGOUT,
            slug: refreshToken,
            data: null,
        });
    };

    const customGetNoti = async (
        indexPage: any,
        accessToken: any,
        axiosJWT: any
    ) => {
        return await ClientServices.getNotification(
            indexPage,
            accessToken,
            axiosJWT
        );
    };
    const { isLoading, result, hasNextPage } = useLoadingLazy<NotificationType>(
        indexPage,
        customGetNoti
    );
    ...
    const lastPostRef = useCallback(
        (noti: NotificationType) => {
            ...
            intObserver.current = new IntersectionObserver((notis) => {
                if (notis[0].isIntersecting && hasNextPage && !isLoading) {
            ...
            if (noti) intObserver.current.observe(noti);
        ...
        [isLoading, hasNextPage, readNoti]
    );
    ...
    const checkRead = result && result.some((noti) => noti?.isRead === false);
                    ...
                    <div className="flex gap-5 items-center justify-end mx-4">
                        {data.token ? (
                            <>
                                ...
                                <DropdownMenu>
                                    <DropdownMenuTrigger>
                                        <div className="relative shrink-0 rounded-full  p-2.5 bg-input hover:brightness-75 cursor-pointer text-white/80">
                                            {checkRead && (
                                                <div className="absolute bottom-auto left-auto right-1 top-2 z-10 inline-block -translate-y-1/2 translate-x-2/4 rotate-0 skew-x-0 skew-y-0 scale-x-50 scale-y-50 rounded-full bg-pink-700 p-2.5 text-xs"></div>
                                            )}
                                            <svg
                                    ...
                                    <DropdownMenuContent className="w-80 mr-4  dark:text-black font-bold h-96 overflow-y-scroll">
