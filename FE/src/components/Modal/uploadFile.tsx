...
import { uploadAvatarFail } from "@/redux/userSlice";
...
export const UploadFIle: React.FC<UploadFileProps> = ({ setFlag }) => {
  const avatar = useSelector(
    (state: RootState) => state.user.detail.data.avatar
  );

  const [urlImage, setUrlImage] = useState<string>(avatar ? avatar : "");
  ...
  const handleUploadAvatarUser = async (file: File) => {
    try {
      if (file) {
        ...
        await sendRequest({
          type: REQUEST_TYPE.UPLOAD_AVATAR,
          data: formData,
        });

        setUrlImage(avatar ? avatar : "");
      }
    } catch (error) {
              ...
              {avatar ? (
                <div className="relative flex flex-col space-y-3  p-4 text-title-foreground border border-border rounded">
                  ...
                  <div className="flex justify-center w-full ">
                    <Button
                      ...
                      type="button"
                      onClick={() => {
                        dispatch(uploadAvatarFail("")), setUrlImage("");
                      }}
                    >
                      <XCircle />
                    </Button>
                ...
                <>
                  <input
                    type="file"
                    onChange={(event: any) => {
                      handleUploadAvatarUser(event.target.files[0]);
                    }}
                    id="input-file"
                    hidden
                  />
                      ...
                      <div className="flex flex-col items-center justify-center py-10 text-center">
                        {isLoading ? (
                          ...
                          <>
                            ...
                            <p className="m-0">
                              Click in this area to add new post
                            </p>
                          </>
                        )}
                      </div>
