        ...
        if (!ref.current) {
            ...
            editor = new EditorJS({
                ...
                onChange: async () => {
                    ...
                    const result = edjsParser.parse(content as any);
                    setReport((prev): any => ({
                        ...prev,
                        content: result?.join(""),
                    }));
                        ...
                        config: {
                            uploader: {
                                async uploadByFile(file: File) {
                                    ...
                                    const { body } =
                                        await ClientServices.uploadFiles(
                                            file,
                                            accessToken,
                                            axiosJWT
                                        );
    ...
    const render = () => {
        switch (Step) {
            ...
            case "STEP_TWO":
                return (
                    <DetailBlog
                        report={(report as any) || ({} as BlogPost)}
                        setReport={setReport}
                    />
                );
            case "STEP_THREE":
            ...
            <div className=" p-4 w-[1200px]  rounded-lg pt-10 mx-auto text-title space-y-10">
                ...
                <div className="fixed bottom-10 z-50 left-1/2 transform -translate-x-1/2 -translate-y-1/2 space-x-5">
                    <div className="flex gap-5 space-x-5">
                        ...
                        <Modal
                            flag={displayCreate}
                            closeModal={setDisplayCreate.off}
                        >
                            {displayModal ? (
                                <ChangeDraft
